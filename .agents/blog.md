...
**Sprint deux articles par semaine (depuis le 17/08).** Une seconde édition tourne le lundi. Elle ne prend jamais un sujet daté ni l'exception actualité (ils restent au jeudi) et choisit dans cet ordre : sujets « cible » (niche), puis « requête outillée », puis « coulisses ». L'édition du jeudi garde ses règles normales. Fin du sprint : quand moins de 8 sujets non cochés restent ici, l'édition du lundi ne publie plus et demande sa désactivation ; on repasse à un article par semaine.

- [x] → 20/08 : Charte IA d'entreprise : le guide et le modèle à télécharger — `charte-ia-entreprise`, modèle livré en `public/charte-ia-modele.md` (méthode, guide de référence ; angle vide confirmé par la recherche SEO du 2026-08-10). Requêtes visées : « charte IA entreprise », « modèle charte IA ». Définition extractible dès le chapô. L'article 4 de l'AI Act en appui (lier `ai-act-pme-obligations`). Les 8 à 10 clauses qu'une charte de PME doit trancher : usages autorisés, données interdites de saisie, validation humaine, outils approuvés, confidentialité client, formation. Le modèle complet livré en fichier `public/charte-ia-modele.md`, téléchargeable librement, jamais derrière un formulaire (les moteurs IA ne citent pas ce qu'ils ne peuvent pas lire), lié depuis l'article et ajouté à `public/llms.txt`. FAQ : « Une charte IA est-elle obligatoire ? », « Qui doit la signer ? », « Que risque une PME sans cadre d'usage de l'IA ? ». Fin d'article : renvoi vers le kit de démarrage.
...
## Changelog
- v1.4 (2026-08-14) : sprint temporaire à deux articles par semaine, édition du lundi ajoutée (routine cloud dédiée) avec sa règle de choix (niches, puis outillé, puis coulisses) et sa condition d'arrêt (moins de 8 sujets restants).
