...
## Overview
Coucou IA is an AI consulting company for the French market, based in Nice. Targets (repositioned 2026-08-12): small fast-deciding businesses in niches where generalist AI falls short because the work lives in métier tools. Priority sectors: immobilier, location saisonnière/conciergeries, hôtellerie indépendante, artisans du bâtiment, yachting; then expertise comptable, services B2B, industrie. ETI is no longer a headline target. The offer has three temps plus two entry doors: **Trouver** (Audit & stratégie IA: états des lieux, cartographie d'opportunités, business cases ROI, roadmaps), **Construire** (Développement IA sur mesure: agents IA, RAG, automatisations, intégrations), **Faire tourner** (suivi mensuel: surveillance, corrections, évolutions), and two small offers with published delays (never published prices): **le premier agent** (one use case, in production in 3 weeks) and **la journée de formation** (one day, the client's real cases). Local SEO pages live at /consultant-ia-{nice,sophia-antipolis,monaco,aix-marseille}; OpenClaw and Hermes Agent are analyzed AND installable offers on /agents-ia. Differentiator: **ouvrir le champ des possibles**: we show non-expert dirigeants what AI makes possible in their business and where to start, sans jargon, then ship it to production. ROI survives as method (business case chiffré avant d'engager, résultats mesurés), never as a guarantee: the word "garanti" is banned from all copy. The word "diagnostic" is banned too (medical connotation): the discovery call is called **"le point de départ"** (fallback noun in prose: **"ce premier échange"**). The site has one job: build credibility (light enterprise-tech, ink on near-white) and convert visitors into **booked "point de départ" calls**. Pricing is never published. 100% French, `vous`, no i18n.
...
3. **Accent discipline.** TWO accents, cleanly split. Deep electric blue `--primary` owns every signal: CTAs, key metrics, focus rings, links, the carte's rays. Warm coral `--accent-2` is the wink: carte marks, ~6% of flow-field particles, the closing glow; never on actions, metrics, links, or body text. No eyebrows anywhere: the mono kicker label above a heading is retired. Never a third accent (the violet stays retired). Neutrals: warm surfaces (hue 85), cool ink text; never blue-grey surfaces or washed-out grey text.
