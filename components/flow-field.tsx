...
// Hero background: a flow-field of 200 particles leaving fading trails. The
// base strokes are a faint blue haze (--primary at low alpha, not grey: grey
// trails read as dirt on the light page), ~10% run brighter blue and ~6%
// warm coral (--accent-2, the wink accent).
...
export function FlowField({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const baseStroke = rgba("--primary", 0.14);
    const blueStroke = rgba("--primary", 0.34);
    const warmStroke = rgba("--accent-2", 0.3);

    let particles: { x: number; y: number; blue: boolean; warm: boolean }[] =
      [];
    let t = 0;
    ...
    const step = () => {
      ...
      for (const p of particles) {
        ...
        ctx.strokeStyle = p.blue ? blueStroke : p.warm ? warmStroke : baseStroke;
        ctx.beginPath();
        ctx.moveTo(p.x, p.y);
        ctx.lineTo(nx, ny);
        ctx.stroke();
